...
import { withCrossSeedRuntime } from "./startup.js";
import {
...
program
	...
	.action(async (searchee, candidate) => {
		await diffCmd(searchee, candidate);
		await db.destroy();
		await memDB.destroy();
	});
...
createCommandWithSharedOptions("daemon", "Start the cross-seed daemon")
	...
	.addOption(apiKeyOption)
	.action(
		withCrossSeedRuntime(async (options) => {
...
createCommandWithSharedOptions("search", "Search for cross-seeds")
	...
	.action(withCrossSeedRuntime(bulkSearch));
...
).action(withCrossSeedRuntime(restoreFromTorrentCache));
